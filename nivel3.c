...
int execute_line(char *line){
    ...
    if (check_internal(args)==0){
        ...
        if (pid==0){
             ...
             printf("execute_line()→Soy el hijo. PID: %d \n",pid);
            execvp(args[0],args);
                         
            printf("execute_line()→ERROR: %s \n",strerror(errno));
            exit(-1);
            
        }else if (pid>0){
            printf("execute_line()→Soy el padre. PID: %d \n",pid);
            pid_t pid_acabado;
            //padre
           ...
           printf("execute_line()→CMD es igual: %s",jobs_list->cmd);
           printf("\n");
        
          
        pid_acabado=wait(&estado_h);
        printf("execute_line()→El hijo terminó \n");
        printf("execute_line()→Estado al terminar: %d \n",estado_h);
        printf("execute_line()→Mi shell es:%s  \n",mi_shell);
        //devolvemos el jobs_list[0] a sestado anterior
...
int internal_source(char **args) {
    ...
    char script_line[COMMAND_LINE_SIZE];
    while (fgets(script_line, COMMAND_LINE_SIZE, file) != NULL) {
        size_t length = strlen(script_line);
        if (script_line[length - 1] == '\n') {
            script_line[length - 1] = '\0'; // Replace newline with null character
        }

        // Execute each line from the script file
